...
export const SecondColumn = ({name,productId,description}: {name: string;description:string;productId: string;}) => {
  return (
    <div className='sm:w-48'>
      <Link href={`/products/${productId}`}>
        <h3 className='capitalize font-medium hover:underline'>{name}</h3>
      </Link>
      {description && <h5 className='mt-2 capitalize text-xs'>{description}</h5>}
    </div>
  );
};
